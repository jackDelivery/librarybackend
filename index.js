...
app.get("/books", async (req, res) => {
  try {
    const users = await Books.find(
      {},
      "carname seater mileage company color model borowedby price status images"
    );
    res.status(200).json(users);
  } catch {
    res.status(500).json({ error: "Internal server error" });
...
app.post("/userequest", async (req, res) => {
  try {
    const { fullname, address, phone, email, accountno, status, reqfor } =
      req.body;
    console.log(req.body);
...
app.put("/userequest/:id", async (req, res) => {
  ...
  try {
    ...
    res
      .status(201)
      .json({ message: "Request updated successfully", updatedBook });
  } catch (error) {
    console.error(error);
    res.status(500).json({ error: "Internal server error" });
  }
...
app.get("/userequest", async (req, res) => {
  try {
    const Request = await Requests.find(
      {},
      "fullname address phone email accountno status reqfor"
    );
